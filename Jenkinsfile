      ...
      steps {
        sh '''
          ...
          docker run --rm \
            ...
            bash -ec 'mkdir -p "$HOME" "$PIP_CACHE_DIR" && export PATH="$HOME/.local/bin:$PATH" && python -m pip install --upgrade pip && python -m pip install --no-cache-dir -r requirements.txt && python -m prisma generate --schema prisma/schema.prisma && python -m pytest tests/unit -q -m unit'
        '''
      }
    }
