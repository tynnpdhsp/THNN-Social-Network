...
const Messaging = ({ onViewProfile, preselectedUser }) => {
  const { user, token, onlineUsers } = useAuth();
  ...
  const openChat = useCallback(
    async (conv) => {
      ...
      try {
        // Mark as read
        apiFetch(`/messaging/conversations/${conv.id}/read`, {
          method: "POST",
        });
  ...
  const startChat = useCallback(
    async (targetUser) => {
      ...
      let conv = conversations.find(
        (c) =>
          c.type === "direct" &&
          (c.other_member?.id === targetUser.id ||
            c.other_member?.userId === targetUser.id),
      );
  ...
  const handleUpload = async (e) => {
    ...
    for (let f of files) {
      ...
      fd.append("file", f);
      try {
        const res = await apiFetch("/social/media/upload", {
          method: "POST",
          body: fd,
        });
        if (res.ok) {
  ...
  const searchUsers = async () => {
    ...
    const res = await apiFetch(
      `/account/search?query=${encodeURIComponent(searchQuery)}`,
    );
    const data = await res.json();
  ...
  const getChatName = (conv) => {
    ...
    if (conv.type === "direct" && conv.other_member)
      return conv.other_member.full_name;
  ...
  return (
    <div className="messaging-container" style={s.container}>
      ...
      <div
        ...
        style={{ ...s.sidebar, ...(activeConv ? { display: "none" } : {}) }}
      >
              ...
              {searchResults.map((u) => (
                ...
                >
                  ...
                  >
                    <img
                      src={
                        resolveImageUrl(u.avatar_url) ||
                        getDefaultAvatar(u.full_name)
                      }
                      alt=""
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          {conversations.filter((c) => {
            ...
            <div
              style={{ textAlign: "center", padding: 40, color: "var(--ash)" }}
            >
              <MessageSquare
                size={32}
            ...
            conversations
              ...
              .map((c) => {
                ...
                const lastRead = member
                  ? member.last_read_at || member.lastReadAt
                  : null;
                const isUnread =
                  lastMsg &&
                  (!lastRead ||
                    new Date(lastMsg.created_at || lastMsg.createdAt) >
                      new Date(lastRead));
                  ...
                  <div
                    ...
                    onMouseLeave={(e) => {
                      e.currentTarget.style.background =
                        activeConv?.id === c.id ? "var(--surface-card)" : "";
      ...
      <div
        ...
        style={{ ...s.chatArea, ...(activeConv ? {} : { display: "none" }) }}
      >
        {activeConv && (
          <>
            {/* Chat Header */}
                ...
                <img
                  src={
                    activeConv.type === "direct"
                      ? resolveImageUrl(activeConv.other_member?.avatar_url) ||
                        getDefaultAvatar(activeConv.other_member?.full_name)
                      : getDefaultAvatar(activeConv.name)
                  }
                  alt=""
                  style={{
                    width: 36,
                    ...
                    objectFit: "cover",
                    cursor:
                      activeConv.type === "direct" ? "pointer" : "default",
                  }}
                  onClick={() =>
                    activeConv.type === "direct" &&
                    onViewProfile?.(
                      activeConv.other_member?.id ||
                        activeConv.other_member?.userId,
                    )
                  }
                />
                <div>
                  ...
                  <p
                    style={{
                      ...
                      color: onlineUsers.includes(
                        String(activeConv.other_member?.id),
                      )
                        ? "#16a34a"
                        : "#9CA3AF",
                      fontWeight: 600,
                    ...
                    {!isMe && (
                      <img
                        src={
                          activeConv.type === "direct"
                            ? resolveImageUrl(
                                activeConv.other_member?.avatar_url,
                              ) ||
                              getDefaultAvatar(
                                activeConv.other_member?.full_name,
                              )
                            : getDefaultAvatar("?")
                        }
                        alt=""
                    ...
                    >
                      ...
                      >
                        ...
                        {isMe &&
                          activeConv.type === "direct" &&
                          (() => {
                            const other = activeConv.members?.find(
                              (mem) => (mem.user_id || mem.userId) !== user.id,
                            );
                            const lastRead =
                              other?.last_read_at || other?.lastReadAt;
                            if (
                              lastRead &&
                              new Date(lastRead) >= new Date(createdAt)
                            ) {
                              return (
                                <span
                                  style={{
            ...
            <div style={s.chatInputRow}>
              <label
                style={{ cursor: "pointer", color: "var(--ash)", padding: 8 }}
              >
                <Image size={20} />
                <input
                  type="file"
